...
import { CircleXIcon, FolderIcon, FolderOpenIcon, FilterIcon } from "lucide-react";
...
export default function Component() {
  ...
  useEffect(() => {
    ...
    const directMatches = allItems.filter(item => {
    ...
    <div className="flex flex-col gap-2 h-full *:nth-2:grow">
        ...
        <Input 
          ...
          onBlur={(e) => {
            ...
            if (searchValue && searchValue.length > 0) {
              const searchProps = tree.getSearchInputElementProps();
              if (searchProps.onChange) {
                const syntheticEvent = { target: { value: searchValue } } as React.ChangeEvent<HTMLInputElement>;
      ...
      <Tree indent={indent} tree={tree}>
        {searchValue && filteredItems.length === 0 ? (
          <p className="py-4 px-3 text-center text-sm">
