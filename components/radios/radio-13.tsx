    ...
    <fieldset className="space-y-4">
      ...
      <RadioGroup className="grid grid-cols-3 gap-2" defaultValue="r1">
        {items.map((item) => (
          <label
            key={item.id}
            className="relative flex cursor-pointer flex-col items-center gap-3 rounded-lg border border-input px-2 py-3 text-center shadow-sm shadow-black/5 transition-colors has-[[data-disabled]]:cursor-not-allowed has-[[data-state=checked]]:border-ring has-[[data-disabled]]:opacity-50 has-[[data-state=checked]]:bg-accent outline-offset-2 has-[:focus-visible]:outline has-[:focus-visible]:outline-2 has-[:focus-visible]:outline-ring/70"
          >
            <RadioGroupItem
              id={item.id}
              value={item.value}
