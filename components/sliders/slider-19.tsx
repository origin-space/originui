...
export default function Slider19() {
  ...
  const itemCounts = Array(tick_count).fill(0).map((_, tick) => {
  ...
  const isBarInSelectedRange = (
    ...
    return countItemsInRange(sliderValue[0], sliderValue[1]) > 0 && rangeMin <= sliderValue[1] && rangeMax >= sliderValue[0];
  };   
